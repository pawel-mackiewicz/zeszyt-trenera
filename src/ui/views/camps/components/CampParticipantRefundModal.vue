...
import {
  ...
  type CampParticipantRefundSubmit
} from './campParticipantActionModalUtils'

type FormErrorKey = 'invalidRefund' | 'refundExceedsAvailable'
...
const emit = defineEmits<{
  close: []
  submit: [payload: CampParticipantRefundSubmit]
}>()
...
function resetForm() {
  refundAmount.value = formatMoneyInput(props.refundableBalance)
  refundNote.value = ''
...
function submitRefund() {
  ...
  if (!amount) {
    formError.value = 'invalidRefund'
    return
  }

  if (amount.amountMinor > props.refundableBalance.amountMinor) {
  ...
  emit('submit', {
    amount,
    note: optionalText(refundNote.value)
  })
}
</script>
  ...
  >
    ...
    >
      ...
      <label
        ...
        for="campParticipantRefundAmount"
      >
        {{ t('fields.refundAmount') }}
      </label>
      <input
        id="campParticipantRefundAmount"
      ...
      <label
        class="app-section-label camp-participant-action-modal__field-label"
        for="campParticipantRefundNote"
      >
        {{ t('fields.refundNote') }}
      </label>
      <input
        id="campParticipantRefundNote"
        v-model="refundNote"
        class="camp-participant-action-modal__control"
        type="text"
      />
    ...
    <template #actions>
      <AppButton
        ...
        type="submit"
        form="campParticipantRefundForm"
...
<i18n lang="json">
{
  "pl": {
    "actions": {
      "cancel": "Anuluj",
      "confirmRefund": "Zapisz zwrot",
      "submitting": "Zapisywanie..."
    },
    "errors": {
      "invalidRefund": "Podaj dodatnią kwotę zwrotu.",
    ...
    "modals": {
      "refundTitle": "Zarejestruj zwrot"
    }
  },
  "en": {
    "actions": {
      "cancel": "Cancel",
      "confirmRefund": "Save refund",
      "submitting": "Saving..."
    },
    "errors": {
      "invalidRefund": "Enter a positive refund amount.",
